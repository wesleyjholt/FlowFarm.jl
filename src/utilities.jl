...
# function calculate_wakecount(turbine_x, turbine_y, rotor_diamter, winddirection)
#     """
...
#     Args:
#         turbineX (numpy.ndarray): x-coordinates for turbines in the farm
#         turbineY (numpy.ndarray): y-coordinates for turbines in the farm
#         wd (float): Wind direction (in meteorological coordinate system) for determining waked turbines.
#     Returns:
#         list(int): N
#     """
...
# function plotFarm(turbine_x, turbine_y, boundary_vertices::Array{Float64}; windfield=false, wakecount=false)
...
# sorted_turbine_index, ct_models, rotor_sample_points_y, rotor_sample_points_z, windresource,
# model_set, wind_farm_state_id=1, velocity_only=false)

# # calculate the power production of each wind turbine
# # ff.turbine_powers_one_direction(generator_efficiency, cut_in_speed, cut_out_speed, rated_speed, rated_power, rotor_diameter, turbine_inflow_velcities, air_density, power_models)

# if windfield
#     # set up the point locations to be used in generating the plot
#     stepsize = 5
#     xrange = minimum(turbine_x)-2*rotor_diameter:stepsize:maximum(turbine_x)+2*rotor_diameter
#     yrange = minimum(turbine_y)-2*rotor_diameter:stepsize:maximum(turbine_y)+2*rotor_diameter
#     zrange = minimum(hub_height):stepsize:maximum(hub_height)
...
#     rotor_diameter, hub_height, turbine_local_ti, sorted_turbine_index, turbine_inflow_velocities,
#     windresource; wind_farm_state_id=1)

#     # visualize the resulting data
#     matplotlib.use("tkagg")
#     plt.contourf(xrange, yrange, velh, cmap="Blues_r")
#     plt.show()
# end